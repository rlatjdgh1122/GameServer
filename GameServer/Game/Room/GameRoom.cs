            ...
            lock (_lock)
            {
                foreach (Item i in _items.Values)
                {
                    if (i.Id == iGetPacket.Iteminfo.ItemId)
                    {
                        ...
                        // iGetPacket 아이템 취득 판정 후 클라로 패킷 전송 
                        S_ItemGet s_ItemGet = new S_ItemGet()
                        {
                            Iteminfo = new()
                        };
                        s_ItemGet.Iteminfo.ItemId = i.Id;
                        player.Session.ProtoSend(s_ItemGet);
                        ...
                        //아이템 획득 후 아이템 사라지게 디스폰 패킷 전송 (브로드캐스트)
                        LeaveGame(i.Id);
